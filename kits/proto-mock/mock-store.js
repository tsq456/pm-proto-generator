...
(function (global) {
  ...
  function mergeMissingSeed(persisted, seed, idKey) {
    const seen = {};
    persisted.forEach(function (row) {
      if (row && row[idKey] != null) seen[String(row[idKey])] = true;
    });
    const out = persisted.slice();
    seed.forEach(function (row) {
      if (!row || row[idKey] == null || seen[String(row[idKey])]) return;
      out.push(clone(row));
    });
    return out;
  }
  ...
  const ProtoMock = {
    ...
    async bootstrap(options) {
      ...
      for (let i = 0; i < names.length; i++) {
        ...
        const rows = !persisted || options.fresh ? clone(seed) : mergeMissingSeed(persisted, seed, idKey);
        STORE[name] = {
          idKey: idKey,
          seed: clone(seed),
          rows: rows,
        };
        if (!persisted || options.fresh || rows.length !== persisted.length) writePersisted(name, rows);
      }
      READY = true;
      return ProtoMock;
    },
