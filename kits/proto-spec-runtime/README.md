...
- 主按钮「原型说明」→ 说明抽屉；可拖动吸附；默认半隐藏，视口侧边移入展开
- **更新记录** → 独立抽屉（本页 | 全部）
...
传入 `demoPerspectives` 时，Runtime 在 FAB **之外**挂一条常驻「评审工具」条：

- 始终可见，不随 FAB 收起；可拖到视口内任意位置（不能拖出视口）
- 白底胶囊样式；按 `group` 展示下拉：`role` / `status` / `scenario`（文案分别为角色 / 状态 / 权限场景）
- 推荐用 `scenario` 表达权限效果（可维护、只读、无模块权限等），避免绑定可变岗位名
- `demoPerspectiveActive`：当前值（字符串或按组对象）；`onDemoPerspective(item)`：切换回调，由页面刷新当前态，不新增页面

未传 `demoPerspectives` 时不渲染工具条。完整字段见 `references/mount-snippet.md`。
