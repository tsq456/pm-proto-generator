...
- [ ] 可在控制台调用 `ProtoMock.reset` 清 store；**禁止**在业务页放「重置数据」等原型专用按钮
- [ ] **用户可见文案**遵守 `references/copywriting.md`：确认框/空态/Toast **不得**出现 Mock、ProtoMock、localStorage、「仅影响原型」「http 静态服务」等词；业务页不出现原型工具控件

## 与 Spec

`interaction.md` / Spec 末尾「原型演示说明」可写 ProtoMock；**业务章节与页面 UI 文案**按正式产品口吻书写，见 `references/copywriting.md`。
